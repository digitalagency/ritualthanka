...
@section('page-header')
    <h1>
        ...
        <small>{{ trans('labels.backend.banner.all_banner') }}</small>
    </h1>
@endsection
    ...
    <div class="content-wrap">
        <div class="row">
            <div class="col-md-8 col-xs-12">
                <div class="box box-success">
                    <div class="box-header with-border">
                        <h3 class="box-title">{{ trans('labels.backend.banner.category') }}</h3>
                    </div>
                    <!-- /.box-header -->
                    ...
                    <div class="box-body">
                        <div class="table-responsive">
                            <table id="article-table" class="table table-condensed table-hover">
                                <thead>
                                <tr>
                                    <th>{{ trans('labels.backend.banner.cattable.name') }}</th>
                                    <th>{{ trans('labels.backend.banner.cattable.slug') }}</th>
                                    ...
                                    @forelse ($postcat as $cat)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <a href="/admin/banner/category/{{$cat->id}}" class="btn btn-xs btn-primary"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" title="Edit"></i></a>
                                                <a data-method="delete" data-trans-button-cancel="Cancel" data-trans-button-confirm="Delete" data-trans-title="Are you sure you want to do this?" class="btn btn-xs btn-danger" style="cursor:pointer;" onclick="$(this).find(&quot;form&quot;).submit();"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete"></i>
                                                    <form action="{{ route('admin.banner.bannercatdestroy',$cat->id) }}" method="POST" name="delete_item" style="display:none">
            ...
            <div class="col-md-4 col-xs-12">
                {{ Form::open(['route' => 'admin.banner.bannercatstore', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post']) }}

                <div class="box box-success">
                    <div class="box-header with-border">
                        <h3 class="box-title">{{ trans('labels.backend.banner.add_category') }}</h3>
                    </div>
                    <!-- /.box-header -->
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-12">
                                <div class="input-group">
                                    ...
                                    {{ Form::text('image', !empty($cat->image) ? $cat->image : '', ['class' => 'form-control', 'id'=>'thumbnail', 'placeholder' => trans('validation.attributes.backend.access.category.photo')]) }}
                                </div>
                                @if(!empty($cat->image))
                                    <img id="holder" style="margin-top:15px;max-height:100px;" src="{{ URL::to($cat->image) }}">
                                @else
