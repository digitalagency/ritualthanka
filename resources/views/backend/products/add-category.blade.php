    ...
    <div class="content-wrap">
        <div class="row">
            <div class="col-md-8 col-xs-12">
                <div class="box box-success">
                    <div class="box-header with-border">
                        <h3 class="box-title">{{ trans('labels.backend.product.category') }}</h3>
                    </div>
                    <!-- /.box-header -->
                    ...
                    <div class="box-body">
                        <div class="table-responsive">
                            <table id="article-table" class="table table-condensed table-hover">
                                <thead>
                                <tr>
                                    <th>{{ trans('labels.backend.product.cattable.name') }}</th>
                                    <th>{{ trans('labels.backend.product.cattable.slug') }}</th>
                                    ...
                                    @forelse ($postcat as $cat)
                                        <tr id="cat_{!! $cat->id !!}">
                                            ...
                                            <td>
                                                ...
                                                <a href="/admin/product/category/{{$cat->id}}" class="btn btn-xs btn-primary"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" title="Edit"></i></a>
                                                ...
                                                <a href="javascript:void(0)" onclick="getcatproducts('{{$cat->id}}')" class="btn btn-xs btn-primary"><i class="fa fa-search" data-toggle="tooltip" data-placement="top" title="View Products"></i></a>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
            ...
            <div class="col-md-4 col-xs-12">
                {{ Form::open(['route' => 'admin.product.catstore', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post']) }}

                <div class="box box-success">
                    <div class="box-header with-border">
                        @if(!empty($editcat->id))
                        <h3 class="box-title">{{ trans('labels.backend.product.edit_category') }}</h3>
                        @else
                        <h3 class="box-title">{{ trans('labels.backend.product.add_category') }}</h3>
                        @endif
                    </div>
                    <!-- /.box-header -->
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            {{ Form::label('subcat', trans('validation.attributes.backend.access.category.parcat'),
                             ['class' => 'col-lg-12 lft-align control-label']) }}
                            ...
                            <div class="col-lg-12">
                                ...
                                <select class="form-control" name="subcat" id="subcat">
                                    <option value="0">--{{trans('validation.attributes.backend.access.category.parcat')}}--</option>
                                    @if(!empty($postcat) and $postcat->count()>0)
                                        @foreach ($postcat as $cat)
                                            @if($cat->parent=="0")
                                                @if(!empty($editcat))
                                                    <option value="{{$cat->id}}" @if($cat->id==$editcat->parent) selected  @endif @if($cat->id==$editcat->id) disabled  @endif>{{$cat->name}}</option>
                                                @else
                        ...
                        <!--form control-->
                            ...
                            <div class="form-group">
                                {{ Form::label('description', trans('validation.attributes.backend.access.category.description'),
                                 ['class' => 'col-lg-12 lft-align control-label']) }}

                                <div class="col-lg-12">
                                    {{ Form::textarea('description', !empty($editcat) ? $editcat->description : '', ['class' => 'form-control', 'rows' => 4, 'cols' => 10]) }}
                                </div>
                                <!--col-lg-12-->
                            </div>
                            <!--form control-->
...
@section('after-scripts')
    ...
    <script>
        ...
        function getcatproducts(id){
            ...
            $.ajax({
                type: 'POST',
                url: '{{route("admin.product.getcatproducts")}}',
                data: {catid: catid},
                success: function( response ) {
                    console.log(response)
//                    alert(response);
                    $('table .relatedproducts').remove();
                   $("#cat_"+catid).after(response);
                }
            });
        }
    </script>
