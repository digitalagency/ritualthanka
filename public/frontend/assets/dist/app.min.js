...
app.header = {
	...
	bindSCActions: function() {
		app.doc
			.on( 'scroll', function() {
				if ( 110 < ( app.doc.scrollTop() ) ) {
					$( '.site-header' ).addClass( 'main-navigation-shrink' );
				} else {
					$( '.site-header' ).removeClass( 'main-navigation-shrink' );
				}
			});
		...
		$(document).ready(function(){
		    $(".current-menu-item").parent('.dropdown-menu').css({"display": "block"});
		});

		//User setting on click
		$('.user i').click(function() {
			$('.user_setting').slideToggle();
		});

		//togglable cart
		$('.cart').click(function() {
			$('.cart-header').slideToggle();
		});
	},
};
